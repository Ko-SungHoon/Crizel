...
<script type="text/javascript">	
		...
		var chkMsg = "";
		var chkNum = /^(?:[0-9]{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[1,2][0-9]|3[0,1]))-[1-4][0-9]{6}/;
		var nextTo = true;
		
		/** form 태그 안의 모든 text태그들의 값을 가져와서 주민번호 입력여부를 확인한다. **/
		for(var i=0; i<$("#form input:text").length; i++){
			text.push($("#form input:text").eq(i).val());
			if(text[i].match(chkNum) != null){
				chkMsg += text[i].match(chkNum) + "\n";							//출력할 메시지에 주민등록번호를 포함시킨다.
				nextTo = false;													//주민등록번호가 포함되어 있을 시
			}
		} 		
		
		/** 관리자 계정일시 에디터의 내용을 가져와서 주민등록번호 포함여부를 체크한다. **/
		if(document.getElementById("dataContent")!=null){						//dataContent 존재여부 확인
		<%if(bm.isManager()){%>
		oEditors.getById["dataContent"].exec("UPDATE_CONTENTS_FIELD", []);		//에디터의 내용을 dataContent로 이동시키는 것 같음.

		if($("#dataContent").val().replace(/(<([^>]+)>)/gi,"")
				.replace("&nbsp;","").match(chkNum) != null){
			chkMsg += $("#dataContent").val().replace(/(<([^>]+)>)/gi,"")
			.replace("&nbsp;","").match(chkNum) + "\n";							//출력할 메시지에 주민등록번호를 포함시킨다.
			nextTo = false;														//주민등록번호가 포함되어 있을 시
		}
		/** 관리자 계정이 아닐시 dataContent의 내용을 가져와서 주민등록번호 포함여부를 체크한다. **/
		<%}else{%>
		if($("#dataContent").val().match(chkNum) != null){
			chkMsg += $("#dataContent").val().match(chkNum) + "\n";				//출력할 메시지에 주민등록번호를 포함시킨다.
			nextTo = false;														//주민등록번호가 포함되어 있을 시
		}
		<%}%>
		}
		if(nextTo == false){
			chkMsg += "\n위의 텍스트가 주민/외국인등록번호로 인식되었습니다.";
			alert(chkMsg);
			return false;
		}
		return true;
...
<caption>게시글 입력</caption>
			...
			if(bm.isBoardIsSecretCf())
			{
				...
				<tr class="blind">
					...
					<td><input type="checkbox" name="dataSecret"  id="dataSecret" value="true" checked /> 비밀글</td>
				</tr>
				<%
			}%>
			<%
...
<% if(bm.isWriteItem("USER_EMAIL")) { %>
            ...
            <tr>
                <th scope="row"><label for="tmpField1">주민등록번호</label></th>
                <td><input type="text" name="tmpField1" value="<%=EgovStringUtil.isNullToString(bm.getTmpField1())%>" style="width:50%" id="tmpField1" /></td>
            </tr>
            <% } %>
