		...
		<tbody>
			...
			<tr>
				...
				<td>
					...
					%>
						<button class="btn small edge green" type="button" onclick="applySubmit('<%=ob.req_no%>', '<%=ob.apply_flag%>', '<%=ob.inst_no%>')">승인</button>
					<%
					}else if("Y".equals(ob.apply_flag)){
					%>
						<button class="btn small edge white" type="button" onclick="applySubmit('<%=ob.req_no%>', '<%=ob.apply_flag%>', '')">취소</button>
					<%
