<%
/**
*   PURPOSE :   심화 프로그램 insert action page
...
if("insert".equals(mode)){	//******************************** 추가 **************************************************************
	...
	try{
		...
		sql.append("INSERT INTO ART_PRO_DEEP(									");
		sql.append("	PRO_NO,													");
		sql.append("	PRO_CAT,												");
		...
		sql.append("						'ART_PRO_DEEP',						");		//PRO_CAT
		sql.append("						?,									");		//PRO_CAT_NM
		sql.append("						?,									");		//PRO_NAME
		sql.append("						?,									");		//PRO_TCH_NAME
		sql.append("						'',									");		//PRO_TCH_TEL
		...
		if(result>0){
			...
			out.println("location.replace('/program/art/admin/programDeepInsertPopup.jsp');");
			out.println("</script>");
		}
	}
	...
	try{
		sql = new StringBuffer();
		sql.append("UPDATE ART_PRO_DEEP				");
		...
		if(result>0){
			out.println("<script>");
			out.println("alert('정상적으로 처리되었습니다.');");
			out.println("location.replace('/program/art/admin/deepMng.jsp');");
			out.println("</script>");
		}		
	}
	...
	try{
		...
		if(result>0){
			...
			out.println("alert('정상적으로 처리되었습니다.');");
			out.println("opener.location.reload();");
			out.println("location.replace('/program/art/admin/programDeepInsertPopup.jsp?mode=update&pro_no="+pro_no+"');");
			out.println("</script>");
		}
	}
}
