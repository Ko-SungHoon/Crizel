...
if (sessionManager.getName().trim().equals("") || sessionManager.getId().trim().equals("") || sessionManager.getName().trim().length() < 1 || sessionManager.getId().trim().length() < 1) {
    ...
    out.println("location.href='/index.gne?menuCd=DOM_000000126002004002';");
    out.println("</script>");
}
...
if ("ins".equals(dataType)) {
    ...
    //방금 입력한 REQ_NO 추출
    sql     =   new StringBuffer();
    sql.append(" SELECT MAX(REQ_NO), APPLY_FLAG AS REQ_NO FROM ART_REQ_DEEP ");
    ...
    if(result > 0){
        ...
        out.println("location.href='/index.gne?menuCd=DOM_000000126002004005&req_no="+ reqData.req_no +"';");
        out.println("</script>");
    } else {
        out.println("<script>");
...
//update
    ...
    if(result > 0){
        out.println("<script>");
        out.println("alert('정상적으로 처리되었습니다.');");
        out.println("location.href='/index.gne?menuCd=DOM_000000126002004005&req_no="+ req_no +"';");
        out.println("</script>");
    } else {
        out.println("<script>");
        out.println("alert('저장되지 않았습니다. 관리자에게 문의하세요.');");
