...
try {
    ...
    sql_str =   " SELECT ";
    sql_str +=  " CASE ";
    sql_str +=  " WHEN (SELECT NVL(COUNT(*), 0) FROM HAPPY_REQ_SCH WHERE APPLY_FLAG IN ('Y', 'N') AND REQ_SCH_ID = ? AND REQ_DATE = ?) > 0 THEN 'N' ";
    sql_str +=  " WHEN (SELECT NVL(COUNT(*), 0) FROM HAPPY_REQ_SCH WHERE APPLY_FLAG IN ('Y', 'N') AND REQ_DATE = ? GROUP BY REQ_SCH_ID) > 1 THEN 'N' ";
    sql_str +=  " WHEN NVL((SELECT NVL(SUM(MAX_PER), 0) FROM HAPPY_PRO_SCH WHERE SHOW_FLAG = 'Y' AND DEL_FLAG != 'Y') ";
    sql_str +=  "     - (SELECT NVL(SUM(REQ_CNT), 0) FROM HAPPY_REQ_SCH WHERE APPLY_FLAG = 'Y' AND REQ_DATE = ?), 0) <= 0 THEN 'N' ";
    sql_str +=  " ELSE 'Y' ";
