<%
/**
*   PURPOSE :   지역민 평생학습 프로그램 insert action page
...
if ("ins".equals(dataType)) {
    ...
    sql_str =   " INSERT INTO HAPPY_REQ_LOCAL ";
    sql_str +=  " ( ";
    sql_str +=  " REQ_NO ";
    sql_str +=  " , PRO_NO ";
    sql_str +=  " , REQ_USER_ID ";
    ...
    sql_str +=  " VALUES ( ";
    sql_str +=  " (SELECT NVL(MAX(REQ_NO) + 1, 1) FROM HAPPY_REQ_LOCAL) "; //REQ_NO
    sql_str +=  " , ? ";    //PRO_NO
    sql_str +=  " , ? ";    //REQ_USER_ID
    sql_str +=  " , ? ";    //REQ_USER_NM
    sql_str +=  " , ? ";    //REQ_USER_TEL
    ...
    //방금 입력한 REQ_NO 추출
    sql     =   new StringBuffer();
    sql.append(" SELECT MAX(REQ_NO) AS REQ_NO, COUNT(REQ_NO) AS APPLY_FLAG FROM HAPPY_REQ_LOCAL ");
    
    reqData =   jdbcTemplate.queryForObject(sql.toString(), new ReqDataList());

    sql     =   new StringBuffer();
    sql_str =   " UPDATE HAPPY_PRO_LOCAL SET ";
    sql_str +=  " CURR_PER = ( ";
    sql_str +=  "   (SELECT CURR_PER FROM HAPPY_PRO_LOCAL WHERE PRO_NO = (SELECT PRO_NO FROM HAPPY_REQ_LOCAL WHERE REQ_NO = ?)) ";
    sql_str +=  "   + (SELECT REQ_PER FROM HAPPY_REQ_LOCAL WHERE REQ_NO = ?) ";
    sql_str +=  " ) ";
    sql_str +=  " WHERE PRO_NO = (SELECT PRO_NO FROM HAPPY_REQ_LOCAL WHERE REQ_NO = ?) ";
    sql.append(sql_str);
    ...
    if(result > 0){
        ...
        out.println("location.href='/index.gne?menuCd=DOM_000000139008003005&req_no="+ reqData.req_no +"';");
        out.println("</script>");
    } else {
        out.println("<script>");
        out.println("alert('저장되지 않았습니다. 관리자에게 문의하세요.');");
        out.println("location.href='/index.gne?menuCd=DOM_000000139008003002';");
        out.println("</script>");
    }
...
//update
    ...
    sql     =   new StringBuffer();
    sql_str =   " UPDATE HAPPY_REQ_LOCAL SET ";
    sql_str +=  " REQ_USER_NM = ? ";
    sql_str +=  " , REQ_USER_TEL = ? ";
    sql_str +=  " , REQ_USER_MAIL = ? ";
    ...
    if(result > 0){
        out.println("<script>");
        out.println("alert('정상적으로 처리되었습니다.');");
        out.println("location.href='/index.gne?menuCd=DOM_000000139008003005&req_no="+ req_no +"';");
        out.println("</script>");
    } else {
        out.println("<script>");
        out.println("alert('저장되지 않았습니다. 관리자에게 문의하세요.');");
...
//apply
    ...
    sql     =   new StringBuffer();
    sql_str =   " SELECT APPLY_FLAG FROM HAPPY_REQ_LOCAL WHERE REQ_NO = ? ";
    sql.append(sql_str);
    ...
    //2nd. HAPPY_REQ_LOCAL apply_flag 변경
    /*if (result > 0) {*/
        result  =   0;
    ...
    if(result > 0){
        out.println("<script>");
        out.println("alert('정상적으로 처리되었습니다.');");
        out.println("location.href='/program/happysch/admin/localReq.jsp';");
        out.println("</script>");
    } else {
        out.println("<script>");
...
//cancel
    ...
    //1st. 취소 자료 승인여부 확인
    sql     =   new StringBuffer();
    sql.append(" SELECT * FROM HAPPY_REQ_LOCAL WHERE REQ_NO = ? ");
