...
<html lang="ko">
	<head>
		<title>RFC관리자 > 프로그램 관리 - 학교연계프로그램</title>
...
sql.append("WHERE CODE_NAME = 'HAPPY_PRO_SCH' 		");
...
function getPopup(type){
	var addr;
	if(type == "artcode"){
		addr = "/program/happysch/admin/programCodePopup.jsp?type=sch";
	}else if( type == "insert"){
...
function updateSubmit(pro_no){
	var addr = "/program/happysch/admin/programSchoolInsertPopup.jsp?mode=update&pro_no="+pro_no;
	newWin(addr, 'PRINTVIEW', '1000', '740');
	//window.open(addr,"PRINTVIEW","width=1000px,height=740px, status=yes, scrollbars=yes, resizable=yes");
}
...
function deleteSubmit(pro_no){
	if(confirm("프로그램을 삭제하시겠습니까?")){
		location.href="/program/happysch/admin/programSchoolInsertAction.jsp?mode=delete&pro_no="+pro_no;
	}else{
		return false;
	}
}
...
<div id="right_view">
		<div class="top_view">
				<p class="location"><strong>프로그램 운영 > 프로그램 관리(학교연계프로그램)</strong></p>
				<p class="loc_admin">
...
<!-- S : #content -->
	<div id="content">
		<div class="btn_area">
			...
			<button type="button" class="btn medium mako" onclick="location.href='/program/happysch/admin/schMng.jsp'">프로그램 관리 - 학교연계프로그램</button>
		...
		<div class="f_r">
			...
			<button type="button" class="btn small edge green" onclick="getPopup('insert');">학교연계 추가</button>
		</div>
		<p class="f_l magT10">
