...
<html lang="ko">
	<head>
		<title>RFC관리자 > 승인대기 및 취소 - 주제별마을학교프로그램</title>
...
sql.append("		FROM HAPPY_REQ_TOWN A LEFT JOIN HAPPY_PRO_TOWN B		");
...
sql.append("			B.MAX_PER < (SELECT NVL(SUM(REQ_PER), 0) FROM (SELECT * FROM HAPPY_REQ_TOWN WHERE APPLY_FLAG = 'Y' AND SHOW_FLAG = 'Y' AND DEL_FLAG != 'Y') WHERE PRO_NO = A.PRO_NO)  ");
sql.append("			THEN 'AE'  					 						");
...
function getPopup(type){
	...
	if(type == "artcode"){
		...
		addr = "/program/happysch/admin/programTownInsertPopup.jsp";
	}
...
function approvalSubmit(req_no){
	var addr = "/program/happysch/client/programTownReqAction.jsp?dataType=app&req_no="+req_no;
	location.href=addr;
    return;
}

function cancelSubmit(req_no){
	var addr = "/program/happysch/client/programTownReqAction.jsp?dataType=can&canAdmin=A&req_no="+req_no;
	location.href=addr;
    return;
}
...
<!-- S : #content -->
	<div id="content">
		<div class="btn_area">
			...
			<button type="button" class="btn medium mako" onclick="location.href='/program/happysch/admin/townReq.jsp'">승인대기 및 취소 - 주제별마을학교프로그램</button>
