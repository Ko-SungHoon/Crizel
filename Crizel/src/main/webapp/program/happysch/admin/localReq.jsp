<%
/**
*   PURPOSE :   승인대기 및 취소 - 지역민 평생교육프로그램
...
<html lang="ko">
	<head>
		<title>RFC관리자 > 승인대기 및 취소 - 지역민 평생교육프로그램</title>
...
sql.append("		FROM HAPPY_REQ_LOCAL A LEFT JOIN HAPPY_PRO_LOCAL B		");
...
sql.append("			B.MAX_PER < (SELECT NVL(SUM(REQ_PER), 0) FROM (SELECT * FROM HAPPY_REQ_LOCAL WHERE APPLY_FLAG = 'Y' AND SHOW_FLAG = 'Y' AND DEL_FLAG != 'Y') WHERE PRO_NO = A.PRO_NO)  ");
sql.append("			THEN 'AE'  					 						");
...
function getPopup(type){
	var addr;
	if(type == "artcode"){
		addr = "/program/happysch/admin/programCodePopup.jsp";
	}else if( type == "insert"){
		addr = "/program/happysch/admin/programLocalInsertPopup.jsp";
	}
...
function approvalSubmit(req_no){
	var addr = "/program/happysch/client/programLocalReqAction.jsp?dataType=app&req_no="+req_no;
	location.href=addr;
    return;
}

function cancelSubmit(req_no){
	var addr = "/program/happysch/client/programLocalReqAction.jsp?dataType=can&canAdmin=A&req_no="+req_no;
	location.href=addr;
    return;
}
...
<div id="right_view">
	<div class="top_view">
		<p class="location"><strong>프로그램 운영 > 승인대기 및 취소(지역민 평생교육프로그램)</strong></p>
		<p class="loc_admin">
...
<!-- S : #content -->
	<div id="content">
		<div class="btn_area">
			...
			<button type="button" class="btn medium mako" onclick="location.href='/program/happysch/admin/localReq.jsp'">승인대기 및 취소 - 지역민 평생교육프로그램</button>
