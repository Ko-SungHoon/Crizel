...
<html lang="ko">
	<head>
		<title>RFC관리자 > 프로그램 관리 - 주제별마을학교프로그램</title>
...
function getPopup(type){
	var addr;
	if(type == "artcode"){
		addr = "/program/happysch/admin/programCodePopup.jsp?type=town";
	}else if( type == "insert"){
...
function updateSubmit(pro_no){
	var addr = "/program/happysch/admin/programTownInsertPopup.jsp?mode=update&pro_no="+pro_no;
	newWin(addr, 'PRINTVIEW', '1000', '740');
	//window.open(addr,"PRINTVIEW","width=1000px,height=740px, status=yes, scrollbars=yes, resizable=yes");
}
...
function deleteSubmit(pro_no){
	if(confirm("프로그램을 삭제하시겠습니까?")){
		location.href="/program/happysch/admin/programTownInsertAction.jsp?mode=delete&pro_no="+pro_no;
	}else{
		return false;
	}
}
...
<!-- S : #content -->
	<div id="content">
		<div class="btn_area">
			...
			<button type="button" class="btn medium mako" onclick="location.href='/program/happysch/admin/townMng.jsp'">프로그램 관리 - 주제별마을학교프로그램</button>
		...
		<div class="f_r">
			...
			<button type="button" class="btn small edge green" onclick="getPopup('insert');">마을프로그램 추가</button>
		</div>
		<p class="f_l magT10">
