...
String search2		=   parseNull(request.getParameter("search2"), "sch");    //학교연계, 지역민 평생, 주제별마을
...
if ("local".equals(search2)) {
    ...
    sql_str +=  " WHERE CODE_TBL = 'HAPPY_PRO_LOCAL' AND CODE_COL = 'PRO_CAT_NM' ";
    ...
    if (Integer.parseInt(search3) > 0) {
        ...
        sql_str +=  " FROM HAPPY_PRO_LOCAL A JOIN HAPPY_PRO_CODE B ";
    ...
    sql     =   new StringBuffer();
    sql_str =   " SELECT * FROM HAPPY_PRO_CODE ";
    sql_str +=  " WHERE CODE_TBL = 'HAPPY_PRO_TOWN' AND CODE_COL = 'PRO_CAT_NM' ";
    sql_str +=  " ORDER BY ORDER1 ";
    sql.append(sql_str);
    proCate =   jdbcTemplate.query(sql.toString(), new ProCatList());
    ...
    if (Integer.parseInt(search3) > 0) {
        sql     =   new StringBuffer();
        sql_str =   " SELECT B.ARTCODE_NO AS PRO_CAT_NO, A.* ";
        sql_str +=  " FROM HAPPY_PRO_TOWN A JOIN HAPPY_PRO_CODE B ";
        sql_str +=  " ON A.PRO_CAT = B.CODE_TBL AND A.PRO_CAT_NM = B.CODE_VAL1 ";
        sql_str +=  " WHERE A.REG_DATE BETWEEN ? AND ? ";
        sql_str +=  " AND B.ARTCODE_NO = ? ";
        sql_str +=  " ORDER BY A.REG_DATE DESC ";
    ...
    sql     =   new StringBuffer();
    sql_str =   " SELECT * FROM HAPPY_PRO_CODE ";
    sql_str +=  " WHERE CODE_TBL = 'HAPPY_PRO_SCH' AND CODE_COL = 'PRO_CAT_NM' ";
    sql_str +=  " ORDER BY ORDER1 ";
    sql.append(sql_str);
    proCate =   jdbcTemplate.query(sql.toString(), new ProCatList());
    ...
    if (Integer.parseInt(search3) > 0) {
        sql     =   new StringBuffer();
        sql_str =   " SELECT B.ARTCODE_NO AS PRO_CAT_NO, A.* ";
        sql_str +=  " FROM HAPPY_PRO_SCH A JOIN HAPPY_PRO_CODE B ";
...
/****** WHERE SETTING *******/
//지역민
if ("local".equals(search2) || "town".equals(search2)) {
...
if ("local".equals(search2)) {
    ...
    sql_str +=  " (SELECT * FROM HAPPY_REQ_SCH ORDER BY REG_DATE DESC) ARA LEFT JOIN HAPPY_REQ_SCH_CNT ARAC ";
...
<body>
	...
	<div id="content">
		<div class="btn_area">
            <button type="button" class="btn medium white" onclick="location.href='/program/happysch/admin/schReq.jsp'">승인대기 및 취소 - 학교연계프로그램</button>
			...
			<button type="button" class="btn medium mako" onclick="location.href='/program/happysch/admin/programStat.jsp'">통계관리</button>
		</div>
		<div class="searchBox magT20 magB20">
			<!-- 개발작업 진행시 검색폼 옵션 및 항목을 수정하시기 바랍니다.-->
			<form action="/program/happysch/admin/programStat.jsp" onsubmit="return searchSubmit()" id="searchForm" method="get" class="topbox2">
				<fieldset>
                    <label for="start_date">신청날짜</label>
					...
					<select id="search2" name="search2" onchange="">
                        <option value="sch" <%if("sch".equals(search2)){out.println("selected");}%>>학교연계프로그램</option>
                        <option value="local" <%if("local".equals(search2)){out.println("selected");}%>>지역민 평생프로그램</option>
                        <option value="town" <%if("town".equals(search2)){out.println("selected");}%>>주제별마을학교프로그램</option>
					</select>
                    ...
                    <%if (("local".equals(search2) || "town".equals(search2)) && Integer.parseInt(search3) > 0) {
                    ...
                    <%}//END FOR
                    } else if ("sch".equals(search2) && Integer.parseInt(search3) > 0) {
...
<script>
    ...
    function proSelAjax (al_deep, pro_cat) {
        ...
        $.ajax({
			type : "POST",
			url : "/program/happysch/client/programSchoolStatAjaxAction.jsp",
    ...
    function excel() {
        ...
        $("#searchForm").attr("action", "/program/happysch/admin/programExcel.jsp");
        $("#searchForm").submit();
        $("#searchForm").attr("action", "/program/happysch/admin/programStat.jsp");
    }
