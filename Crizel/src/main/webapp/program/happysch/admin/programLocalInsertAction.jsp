<%
/**
*   PURPOSE :   지역민 평생교육프로그램 insert action page
...
if("insert".equals(mode)){	//******************************** 추가 **************************************************************
	...
	try{
		...
		sql.append("INSERT INTO HAPPY_PRO_LOCAL(									");
		sql.append("	PRO_NO,													");
		sql.append("	PRO_CAT,												");
		...
		sql.append("						'HAPPY_PRO_LOCAL',					");		//PRO_CAT
		sql.append("						?,									");		//PRO_CAT_NM
		sql.append("						?,									");		//PRO_NAME
		sql.append("						?,									");		//PRO_TCH_NAME
		sql.append("						'',									");		//PRO_TCH_TEL
		...
		if(result>0){
			...
			//out.println("location.replace('/program/happysch/admin/programLocalInsertPopup.jsp');");
			out.println("</script>");
		}
	}
	...
	try{
		sql = new StringBuffer();
		sql.append("UPDATE HAPPY_PRO_LOCAL				");
	...
	try{
		...
		if(result>0){
			...
			out.println("opener.location.reload();");
			out.println("window.close();");
			//out.println("location.replace('/program/happysch/admin/programLocalInsertPopup.jsp?mode=update&pro_no="+pro_no+"');");
			out.println("</script>");
		}
	}
}
