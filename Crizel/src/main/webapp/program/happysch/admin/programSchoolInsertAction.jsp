...
if("insert".equals(mode)){	//******************************** 추가 **************************************************************
	...
	try{
		...
		sql.append("INSERT INTO HAPPY_PRO_SCH(									");
		sql.append("						PRO_NO,								");
		sql.append("						PRO_CAT,							");
		...
		sql.append("						'HAPPY_PRO_SCH',					");		//PRO_CAT
		sql.append("						?,									");		//PRO_CAT_NM
		sql.append("						?,									");		//PRO_NAME
		sql.append("						?,									");		//PRO_MEMO
		sql.append("						?,									");		//PRO_YEAR
		...
		sql.append("FROM HAPPY_BAN_TABLE	");
		...
		if(count <= 0){
			...
			if(banList!=null && banList.size()>0){
				sql = new StringBuffer();
				sql.append("SELECT NVL(MAX(BANNO)+1,1) FROM HAPPY_BAN_TABLE");
				banno = jdbcTemplate.queryForObject(
							sql.toString(),
							Integer.class
						);
				sql = new StringBuffer();
				sql.append("INSERT INTO HAPPY_BAN_TABLE(			");
				sql.append("	BANNO,								");
				sql.append("	YEAR,								");
				sql.append("	BAN_NM,								");
				sql.append("	BAN_DATE,							");
		...
		if(result>0){
			...
			//out.println("location.replace('/program/happysch/admin/programSchoolInsertPopup.jsp');");
			out.println("</script>");
		}
	}
	...
	try{
		sql = new StringBuffer();
		sql.append("UPDATE HAPPY_PRO_SCH			");
		...
		if(result>0){
			out.println("<script>");
			out.println("alert('정상적으로 처리되었습니다.');");
			out.println("location.replace('/program/happysch/admin/schMng.jsp');");
			out.println("</script>");
		}
	}
	...
	try{
		...
		if(result>0){
			...
			out.println("opener.location.reload();");
			out.println("window.close();");
			//out.println("location.replace('/program/happysch/admin/programAlwaysInsertPopup.jsp?mode=update&pro_no="+pro_no+"');");
			out.println("</script>");
		}
	}
}
