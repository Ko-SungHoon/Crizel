...
<html lang="ko">
	<head>
		<title>RFC관리자 > 프로그램 관리 - 지역민 평생교육프로그램</title>
...
sql.append("		FROM HAPPY_PRO_LOCAL	 									");
...
sql.append("WHERE CODE_NAME = 'HAPPY_PRO_LOCAL'		");
...
function getPopup(type){
	var addr;
	if(type == "artcode"){
		addr = "/program/happysch/admin/programCodePopup.jsp?type=local";
	}else if( type == "insert"){
...
function updateSubmit(pro_no){
	var addr = "/program/happysch/admin/programLocalInsertPopup.jsp?mode=update&pro_no="+pro_no;
	newWin(addr, 'PRINTVIEW', '1000', '740');
	//window.open(addr,"PRINTVIEW","width=1000px,height=740px, status=yes, scrollbars=yes, resizable=yes");
}
...
function deleteSubmit(pro_no){
	if(confirm("프로그램을 삭제하시겠습니까?")){
		location.href="/program/happysch/admin/programLocalInsertAction.jsp?mode=delete&pro_no="+pro_no;
	}else{
		return false;
	}
}
...
<div id="right_view">
		<div class="top_view">
				<p class="location"><strong>프로그램 운영 > 프로그램 관리(지역민 평생교육프로그램)</strong></p>
		</div>
</div>
<!-- S : #content -->
	<div id="content">
		<div class="btn_area">
			...
			<button type="button" class="btn medium mako" onclick="location.href='/program/happysch/admin/localMng.jsp'">프로그램 관리 - 지역민 평생교육프로그램</button>
		...
		<div class="f_r">
			...
			<button type="button" class="btn small edge green" onclick="getPopup('insert');">지역민 프로그램 추가</button>
		</div>
		<p class="f_l magT10">
