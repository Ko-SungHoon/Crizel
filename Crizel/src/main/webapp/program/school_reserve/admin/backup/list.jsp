...
try {
	...
	sql.append("AND SCHOOL_TYPE = 'PUBLIC' 								");
	if(!"".equals(keyword)){
...
function schDel(school_id){
	if(confirm("학교정보를 삭제할시 모든 정보가 삭제됩니다.\n삭제하시겠습니까?")){
		location.href="delAction.jsp?school_id="+school_id+"&school_type=PUBLIC";
	}else{
		return false;
	}
...
function selChange(){
	...
	if(length > 0){
		if(confirm("상태를 변경하시겠습니까?")){
			...
			$("#postForm").attr("action", "/program/school_reserve/admin/approvalAction2.jsp?school_type=PUBLIC");
			$("#postForm").submit();
		}else{
			return false;
		}
...
<div id="right_view">
	<div class="top_view">
      <p class="location"><strong>학교관리 - 공립</strong></p>
			...
			<form action="" method="post" id="searchForm">
				<fieldset>
					...
					<div class="btn_area txt_r magT20">
						...
						<button type="button" class="btn medium edge darkMblue" onclick="new_win2('insertForm.jsp?command=insert&sch_type=public','group_form','1000','800','yes','yes','yes');" >학교등록</button>
					</div>
				</fieldset>
			</form>
		</div>
