...
*   MODIFY  :  20180219_mon ljh 마크업 수정
**/
%>
...
function goToList(inst_cat_nm, inst_name, cnt){
	if(cnt>0){
		location.href="/index.gne?menuCd=DOM_000002001003003001&search1="+encodeURIComponent(inst_cat_nm)+"&keyword="+encodeURIComponent(inst_name);
	}else{
	...
	$.ajax({
		...
		success : function(data) {
			$.each(JSON.parse(data), function(i, val) {									//ajax로 받아온 json 데이터를 html로 구성한다
				...
				$("#modal_inst_pic").html("<img src='"+val.inst_pic+"' alt='악기이미지'>");
...
<section class="board">
	...
	%>
		<li>
			<p class="thumb">
				<a href="javascript:;" onclick="javascript:getDetail('<%=ob.inst_no%>')" class="slide_open openLayer" title="<%=ob.inst_name%>"><img src="<%=ob.inst_pic%>" alt="<%=ob.inst_name%>" ></a>
			</p>
			<p class="pro_tit">
				<a href="javascript:;" onclick="javascript:getDetail('<%=ob.inst_no%>')" class="slide_open openLayer"> <%=ob.inst_name %> </a>
			</p>
			<ul class="li_type_dash l fsize_90 magL5">
				<li>악기분류 : <%=ob.inst_cat_nm%></li>
				<li>대여수 / 보유수 : <%=ob.curr_cnt%> / <%=ob.max_cnt%></li>
				<%-- <li>악기위치  : <%=ob.inst_lca%></li>
				<li><%=parseNull(ob.inst_memo).replace("\n", "<br>")%></li> --%>
			</ul>
			<div class="c rent_btn">
				<button type="button" class="btn edge medium darkMblue" onclick="goToList('<%=ob.inst_cat_nm%>', '<%=ob.inst_name%>', '<%=ob.max_cnt - ob.curr_cnt%>')">대여하기</button>
			</div>
		</li>
	...
	<div class="pageing magT20 magB15">
		<%=paging.getHtml("2") %>
	</div>
	<% } %>
...
<div id="slide" style="display:none;">
	...
	<div class="inner">
		<table class="bbs_list2 td-l th-c fsize_80">
			<caption>소장 악기 상세 정보입니다.</caption>
			<colgroup>
				<col style="width:30%" />
				<col />
			</colgroup>
			<tbody>
				<tr>
					...
					<td><span id="modal_inst_cat_nm"></span> </td>
				</tr>
				<tr>
					<th scope="row"> 악기규격 </th>
					<td><span id="modal_inst_size"></span> </td>
				</tr>
				<tr>
					<th scope="row"> 악기이미지 </th>
					<td id="modal_inst_pic"></td>
				</tr>
				<tr>
					<th scope="row"> 악기모델명 </th>
					<td><span id="modal_inst_model"></span></td>
				</tr>
				<tr>
					<th scope="row"> 악기명 </th>
					<td><span id="modal_inst_name"></span></td>
				</tr>
				<tr>
					<th scope="row"> 악기위치 </th>
					<td><span id="modal_inst_lca"></span></td>
				</tr>
				<tr>
					<th scope="row"> 대여량/총량 </th>
					<td><span id="modal_curr_cnt"></span> / <span id="modal_max_cnt"></span></td>
				</tr>
				<tr>
					<th scope="row"> 악기 상세 내용 </th>
					<td>
						<div class="music_detail boxin_roll">
		...
		<div class="btn_area c mg_b5">
			<button type="button" class="btn edge medium darkMblue" id="modal_inst_button">대여하기</button>
		</div>
		<a href="javascript: modal_close();" class="btn_cancel" id="modalClose" title="창닫기"><img src="/img/art/layer_close.png" alt="창닫기"></a>
	</div>
</div>
<%/*************************************** END Modal part ****************************************/%>
