...
String mode				= parseNull(request.getParameter("mode"), "apply");
...
String req_no 			= parseNull(request.getParameter("req_no"), "11");
...
String pageType = parseNull(request.getParameter("pageType"), "client");
	...
	try{
		for(int i=0; i<count; i++){
			...
			if(vo.max_cnt < now_cnt){
				...
				if("admin".equals(pageType)){
					...
					out.println("location.replace('/index.gne?menuCd=DOM_000002001003003002&mode=update&req_no="+req_no+"');");
					//out.println("location.replace('/index.gne?menuCd=DOM_000000126003003001&mode=update&req_no="+req_no+"');");
				}
				out.println("</script>");
				countCheck = false;
				return;
		...
		if(result>0){
			...
			if("admin".equals(pageType)){
				...
				out.println("location.replace('/index.gne?menuCd=DOM_000002001003003001&mode=update&req_no="+req_no+"');");
				//out.println("location.replace('/index.gne?menuCd=DOM_000000126003003001&mode=update&req_no="+req_no+"');");
			}
			out.println("</script>");
		}
	...
	try{
		for(int i=0; i<inst_no.length; i++){
			...
			if(vo.max_cnt < now_cnt){
				out.println("<script>");
				out.println("alert('대여가능한 악기 총량보다 신청 한 수량이 많습니다.');");
				out.println("location.replace('/index.gne?menuCd=DOM_000002001003003000');");
				//out.println("location.replace('/index.gne?menuCd=DOM_000000126003003000');");
				out.println("</script>");
				return;
			}else{
				countCheck = true;
