...
try{
	if("dateList".equals(mode)){
		...
		if(result > 0){
			...
			sql = new String();
			sql += "INSERT INTO ART_BAN_TABLE(BANNO, YEAR, BAN_NM, BAN_DATE, BAN_DATE2, BAN_ETC)	";
			sql += "VALUES((SELECT NVL(MAX(BANNO)+1,1) FROM ART_BAN_TABLE), ?, ?, ?, ?, ?)			";
		...
		sql = new String();
		sql += "DELETE FROM ART_BAN_TABLE WHERE BANNO = ?		";
