...
public class Paging3 {
     ...
     */
    public void setPageNo(int pageNo3) {
    ...
    private void makePaging() {
        ...
        if (this.pageNo3 == 0) this.setPageNo(1); // 기본 값 설정
        if (this.pageSize == 0) this.setPageSize(10); // 기본 값 설정
        if (this.pageBlock == 0) this.setPageBlock(10); // 기본 값 설정

        int finalPage = (totalCount + (pageSize - 1)) / pageSize; // 마지막 페이지
        if (this.pageNo3 > finalPage) this.setPageNo(finalPage); // 기본 값 설정

        if (this.pageNo3 < 0 || this.pageNo3 > finalPage) this.pageNo3 = 1; // 현재 페이지 유효성 체크

        boolean isNowFirst = pageNo3 == 1 ? true : false; // 시작 페이지 (전체)
        boolean isNowFinal = pageNo3 == finalPage ? true : false; // 마지막 페이지 (전체)

        int startPage = ((pageNo3 - 1) / this.pageBlock) * this.pageBlock + 1; // 시작 페이지 (페이징 네비 기준)
        ...
        if (isNowFirst) {
            ...
            this.setPrevPageNo(((pageNo3 - 1) < 1 ? 1 : (pageNo3 - 1))); // 이전 페이지 번호
        }
        ...
        if (isNowFinal) {
            ...
            this.setNextPageNo(((pageNo3 + 1) > finalPage ? finalPage : (pageNo3 + 1))); // 다음 페이지 번호
        }
        ...
        this.setStartRowNo((pageNo3-1) * pageSize);
        this.setEndRowNo(pageNo3 * pageSize);
        this.setRowNo(totalCount - ((pageNo3-1) * pageSize));
    }
    ...
    public String getHtml(String mode) {
    	...
    	html.append("<a class='bt' href=\"javascript:;\" onclick=\"goPage3(");
    	...
    	for(int i=this.startPageNo; i<=this.endPageNo; i++) {
    		if(i == this.pageNo3) {
    			if("2".equals(mode)) {
    				html.append("<a href=\"javascript:;\" class=\"on\" onclick=\"goPage3(");
    			...
    			html.append("<a href=\"javascript:;\" onclick=\"goPage3(");
            	html.append(Integer.toString(i));
            	html.append("); return false;\">");
            	html.append(Integer.toString(i));
            	html.append("</a>");
    	...
    	html.append("<form action=\"\" method=\"post\" id=\"pagingForm3\" name=\"pagingForm3\">");
    	html.append("<input type=\"hidden\" name=\"pageNo3\" value=\"\">");
    	html.append(this.getParams());
    	html.append("</form> \n");
    	
    	html.append("<script type=\"text/javascript\"> \n");
    	html.append("function goPage3(page) { \n");
    	html.append("	document.pagingForm3.pageNo3.value = page; \n");
    	html.append("	document.pagingForm3.submit(); \n");
    	html.append("} \n");
    	html.append("</script> \n");
