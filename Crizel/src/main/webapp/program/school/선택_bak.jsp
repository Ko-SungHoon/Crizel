...
try {
	...
	if(allDate){		
		for(Map<String,Object> ob : typeA){		
			...
			if(!typeBlist.contains(ob.get("date_value").toString())){
				...
				pstmt = conn.prepareStatement(sql.toString());
				pstmt.setString(++key, ob.get("date_value").toString());
				pstmt.setString(++key, room_id);
				rs = pstmt.executeQuery();
				if(rs.next()){
					useY_date_id = rs.getString("DATE_ID");
				}
	...
	for(Map<String,Object> ob : typeB){
		...
		if(typeBlist.contains(ob.get("date_value").toString())){
			...
			sql = new StringBuffer();
			sql.append("SELECT * FROM RESERVE_USE WHERE DATE_VALUE = ? AND DATE_ID = ? ORDER BY DATE_VALUE	");		
			pstmt = conn.prepareStatement(sql.toString());
			pstmt.setString(1, ob.get("date_value").toString());
			pstmt.setString(2, useY_date_id);
			rs = pstmt.executeQuery();
...
$(function(){
	$(".day").click(function(){
		...
		if($.trim(getId) == ""){
			...
			$("#changeForm").attr("action", "/index.gne?menuCd=DOM_000000118003000000");	//테스트서버 		
		}
