...
try {
	...
	for(int k=startNum; k<=lastNum; k++){							//항시개방 날짜, 시간 저장
	...
	for(int i=0; i<timeUseMapList.size(); i++){
		if(timeUseMapList.size()>1){
			if(i+1 < timeUseMapList.size()){
				Map<String,Object> ob = timeUseMapList.get(i);
				Map<String,Object> ob2 = timeUseMapList.get(i+1);
				if(Integer.parseInt(ob.get("use_size").toString()) < Integer.parseInt(ob2.get("use_size").toString())){
					smallNum = Integer.parseInt(ob.get("use_size").toString());
					smallDate_id = ob.get("date_id").toString();
				}else{
					smallNum = Integer.parseInt(ob2.get("use_size").toString());
					smallDate_id = ob2.get("date_id").toString();
				}
	...
	if("".equals(useDate_id)){
		useDate_id = smallDate_id;
	}
	...
	if("기타시설".equals(reserve_type)){
		...
		//군읍면인지 확인
		sql = new StringBuffer();
		sql.append("SELECT RS.AREA_TYPE  ");
	...
	<!-- <span class="red subscript">* 날짜 다중선택이 가능합니다.</span> -->
		<div class="datesel item col-5 mo-col-12">
			<ul id="calDay" class="type03">
				...
				<li>
					시작날짜 : <input type="text" name="date_start" id="date_start" readonly="readonly" value="<%=date_value%>">
				</li>
				<li>
					종료날짜 : <input type="text" name="date_end" id="date_end" readonly="readonly" value="<%=date_value%>"> 
				</li>
			</ul>
			<ul id="checkDiv">
			</ul>
...
%>
		...
		<div class="clr"> </div>
		<div class="c magT20">
				<button type="button" class="btn edge small wps_30 mako" id="dateCheck">예약가능시간 확인</button>
		</div>
	</div>
</section>
...
<div class="c magT30">
	...
	<button type="button" class="btn edge medium white" onclick="location.href='/index.gne?menuCd=DOM_000000106003001000'">취소하기</button>	<!-- 테스트서버 -->
</div>
</form>