...
try{
    ...
    sql_str +=  " FROM HAPPY_REQ_SCH REQAL LEFT JOIN HAPPY_REQ_SCH_CNT REQAL_CNT ON REQAL.REQ_NO = REQAL_CNT.REQ_NO ";
    sql_str +=  " JOIN (SELECT * FROM HAPPY_PRO_SCH WHERE DEL_FLAG != 'Y' AND SHOW_FLAG = 'Y') C ON REQAL_CNT.PRO_NO = C.PRO_NO ";
...
<table class="rwList tb_board nohover thgrey td-c">
	<caption>접수확인 및 취소 목록표로 접수번호, 신청일, 프로그램명, 분류, 총 신청인원, 담당자명, 접수일, 접수상태 등의 정보를 제공합니다.</caption>
	<colgroup>
		<col style="width:8%">
		<col style="width:13%">
		<col />
    ...
    if (reqDateList.size() > 0) {
        for(ArtReqData data : reqDateList){
            ...
            <tr>
                ...
                <td><span class="dis_mo">신청일</span><a href="./index.gne?menuCd=<%=confirmPage%>&req_sch_id=<%=data.req_sch_id %>&req_date=<%=data.req_date %>" class="fb"><%=data.req_date %></a></td>
