...
//	추가된 변수
String advice_cancel	=	"";		//
String advice_sts_tag	=	"";		//
String advice_org_date	=	"";		//최초 신청일 저장 변수
...
try {
	...
	sql.append("SELECT COUNT(*) CNT FROM ADVICE_BOARD_LOG WHERE 1=1 ");
    ...
    sql.append("						   WHEN A.GRADE = 'B03' THEN '고등'         ");
...
/*	sql.append("					,  CASE WHEN A.ADVICE_STS = 'A' THEN '상담대기중'  ");
	...
	sql.append("					   		WHEN A.ADVICE_STS = 'D' THEN '상담대기중 삭제'   ");
	sql.append("					   		WHEN A.ADVICE_STS = 'E' THEN '상담완료중 삭제'   ");
...
*/
	...
	sql.append("					, A.REG_NM                                ");
	sql.append("					, (SELECT MAX(C.BOARD_ID) FROM ADVICE_BOARD_LOG C WHERE C.REF_ID = A.BOARD_ID AND C.BOARD_LVL = 1 AND C.BOARD_DEPT = 1) AS REPLY_ID ");
	sql.append(" 					, A.ADVICE_CANCEL						\n");
	sql.append(" 					, TO_CHAR(TO_DATE(ADVICE_ORG_DATE||ADVICE_ORG_HMS,'YYYYMMDDHH24MISS'),'YYYY-MM-DD HH24:MI:SS') AS ADVICE_ORG_DATE	\n");
	sql.append(" 		FROM ADVICE_BOARD_LOG A  WHERE A.NOTICE_YN = 'N'	\n");
	...
	sql.append("		ORDER BY A.BOARD_ID DESC, A.REG_DT DESC, A.ADVICE_STS ASC ");
...
<div id="right_view">
	...
	<div id="content">
					...
					<div class="boxinner">
						...
						<span>
							...
							<select name="search_advice_sts" id="search_advice_sts" for="search_advice_sts" value="<%=search_advice_sts%>">
								...
								<option <%if("C".equals(search_advice_sts)){ %>selected="selected"<% } %> value="C">상담기간 초과</option>
							</select>
		...
		<div class="btn_area txt_r magT20">
			...
			%>
			<button type="button" onclick="location.href='log.jsp'" class="btn medium edge mako">이전 상담로그</button>
					...
					<table class="bbs_list">
						...
						if(dataList != null && dataList.size() > 0){
							...
							for(int i=0; i<dataList.size(); i++){
								...
								//추가
								advice_sts_tag = parseNull((String)map.get("ADVICE_STS_TAG")); //상태기호
								advice_sts     = parseNull((String)map.get("ADVICE_STS"));     //상태이름
								...
								advice_cancel  = parseNull((String)map.get("ADVICE_CANCEL"));  //취소가 된 이력 확인 Y = 취소된 이력이 있음
								advice_org_date	=	parseNull((String)map.get("ADVICE_ORG_DATE"));  //최초 상담 신청일
						%>
							<tr>
								...
								<%
								/**
								*	PURPOSE	:	상담글이 ADVICE_BOARD 에 존재하는 지 확인 후 없을 경우 link 부여하지 않음
								*	CREATE	:	20171030_mon
								*	MODIFY	:	....
								*/
								/* Board 확인 List<Map> 변수 */
								if (advice_sts_tag.equals("C") || advice_sts_tag.equals("D") || advice_sts_tag.equals("E")) {
								...
								<%
								/**
								*	PURPOSE	:	교육청 요청을 재매칭되는 포인트를 추가함
								*	CREATE	:	20171101_wends	JI
								*	MODIFY	:	20171103_fri	JI	=>	상담신청일 만 표시
								*/
								/*
								if (advice_cancel.equals("Y")) {
									out.println(advice_org_date + "<br>(" + reg_dt + ")");
								} else {
									out.println(reg_dt);
								}
								*/
								if (advice_cancel.equals("Y")) {
									out.println(advice_org_date);
								} else {
									out.println(reg_dt);
								}
								%>
