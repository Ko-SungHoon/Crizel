...
if (sessionManager.getName().trim().equals("") || sessionManager.getId().trim().equals("") || sessionManager.getName().trim().length() < 1 || sessionManager.getId().trim().length() < 1) {
    out.println("<script>");
    out.println("alert(\"로그인한 회원만 신청할 수 있습니다.\");");
...
<script>
	...
	function insertSubmit() {
		...
		for(var i=0; i<cnt; i++){
			...
			if($("#req_inst_cnt_"+i).val() == "0"){
				alert("0 이상의 숫자를 입력하여 주십시오.");
				$("#req_inst_cnt_"+i).focus();
				return false;
			}
		}
	...
	function listSubmit() {
		location.href = "/index.gne?menuCd=<%=listPage%>";	// 악기대여 신청 목록
	}
	...
	$(function(){
		...
		$('input[name=req_inst_cnt]').keyup(function(){this.value = this.value.replace(/[^0-9]/g,'');});
		$('input[name=req_inst_cnt]').change(function(){this.value = this.value.replace(/[^0-9]/g,'');});
	});
</script>
