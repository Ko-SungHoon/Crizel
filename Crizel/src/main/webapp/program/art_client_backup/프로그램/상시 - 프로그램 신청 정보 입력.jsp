...
if (sessionManager.getName().trim().equals("") || sessionManager.getId().trim().equals("") || sessionManager.getName().trim().length() < 1 || sessionManager.getId().trim().length() < 1 || !sessionManager.getGroupId().equals("GRP_000009")) {
	if (sessionManager.isRoleAdmin() || "gne_wwoof2002".equals(sessionManager.getId())) {
		...
		out.println("<script>");
		out.println("alert(\""+sessionManager.getId()+"로그인 정보가 확인되지 않습니다. 학교계정 로그인 후 시도 하세요.\");");
...
if (req_no != null && req_no.length() > 0) {
	...
	sql_str +=  "   WHERE DEL_FLAG != 'Y' AND SHOW_FLAG = 'Y' AND PRO_YEAR = SUBSTR((SELECT REQ_DATE FROM ART_REQ_ALWAY WHERE REQ_NO = ?), 1, 4) ";
	sql_str +=  ") PRO ";
	sql_str +=  "LEFT JOIN ( ";
	sql_str +=  "   SELECT ";
	sql_str +=  "       REQ_AL.* ";
	...
	<div class="box_02 magB20">
		<ul class="type01 ">
			<li><span class="fb">주의사항</span>
				<ul class="fsize_90">
					<li><span class="red">날짜 및 시간 분류(오전반, 오후반, 전일제반)은 수정할 수 없습니다.</span></li>
	...
	<fieldset>
		...
		<table class="table_skin01 fsize td-l f_nanum">
			...
			<tbody>
				...
				<tr>
					<th scope="row"><span class="red fb">*</span> 대표 인솔자명</th>
					...
					<th scope="row"><span class="red fb">*</span> 대표 인솔자 연락처</th>
...
※ 개인정보 수집·이용에 대하여 동의를 원하지 않을 경우 동의를 거부할 수 있으며, 동의 거부시 본 서비스를 이용할 수 없습니다.</textarea>
	...
	<fieldset>
		...
		<div class="btn_area c">
			...
			<button onclick="location.href='./index.gne?menuCd=DOM_000002001002003002'" type="button" class="btn medium edge white">취소</button>
		</div>
	</fieldset>
</form>