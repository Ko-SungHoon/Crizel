...
function insert(){
	location.href="/index.gne?menuCd=DOM_000000126003003001";
}
...
<section class="board">
	...
	<table class="tb_board">
		...
		<tbody>
			...
			<tr>
				<td><%=num--%></td>
				<td><%=parseNull(ob.reg_date) %></td>
				<td><%=parseNull(ob.req_mng_nm) %></td>
				<td><a href="/index.gne?menuCd=DOM_000000126003003002&req_no=<%=ob.req_no%>"><%=parseNull(ob.inst_nm) %></a></td>
				<td><%=parseNull(ob.apply_date) %></td>
				<td><%=parseNull(ob.apply_flag) %></td>
			</tr>
			<%
			}
			}else{
	...
	<div class="search">
		<button onclick="insert();">대여하기</button>
	</div>
